          ...
          {activeAdminTab === 'custom-orders' && (
            <div className="space-y-6">
              <div>
                ...
                <p className="text-xs text-slate-500">Bridal quotes and wholesale requests stored in database</p>
              </div>
                ...
                <div className="space-y-4">
                  {customOrders.map((req) => {
                    ...
                    const handleConfirmBulkOrder = async () => {
                      if (!window.confirm(`Confirm bulk order for ${req.name} (${req.quantity} cones)? This will convert and move it to official Customer Orders.`)) {
                        return;
                      }

                      const newOrderObj = {
                        id: `ORD-BULK-${Math.floor(1000 + Math.random() * 9000)}`,
                        customerName: req.name,
                        phone: req.phone,
                        address: req.location || 'Bulk Delivery Location',
                        city: req.location || 'Bulk Order City',
                        totalAmount: Number(req.estimatedTotal || (req.quantity * (req.pricePerCone || 35))),
                        items: [
                          {
                            id: req.productId || Date.now(),
                            name: req.coneType || 'Organic Henna Cone (Bulk)',
                            price: req.pricePerCone || 35,
                            qty: req.quantity
                          }
                        ],
                        status: 'Confirmed',
                        paymentMethod: 'Custom Invoice / COD',
                      ...
                      try {
                        await createOrder(newOrderObj);
                      } catch (err) {
                        console.warn('Sync order notice:', err);
                      }

                      // Update Custom Order status to Confirmed
                      setCustomOrders(prev => prev.map(c => c.id === req.id ? { ...c, status: 'Confirmed' } : c));

                      alert(`Order ${newOrderObj.id} Confirmed! Redirecting to Orders management tab...`);
                      setActiveAdminTab('orders');
                    };

                    const handleCancelBulkOrder = async () => {
                      if (!window.confirm(`Cancel bulk request for ${req.name}?`)) return;
                      setCustomOrders(prev => prev.map(c => c.id === req.id ? { ...c, status: 'Cancelled' } : c));
                    };

                    return (
                      <div key={req.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-4">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <h4 className="font-bold text-sm text-slate-900 mt-1">{req.name} ({req.phone})</h4>
                          </div>

                          <div className="text-right">
                            <span className="px-3 py-1 bg-indigo-50 text-indigo-700 border border-indigo-200 rounded-full text-[11px] font-bold font-mono">
                              {req.quantity} Cones Requested
                            </span>
                            {req.estimatedTotal > 0 && (
                              <p className="text-xs font-black text-slate-900 mt-1">Est: ₹{req.estimatedTotal}</p>
                            )}
                          </div>
                        </div>

                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-slate-50 p-3 rounded-xl text-xs border border-slate-100">
                          ...
                          <div><span className="text-slate-400 block text-[10px]">Delivery Location</span><strong>{req.location || 'N/A'}</strong></div>
                        </div>

                        {req.specialNotes && (
                          <p className="text-xs text-slate-600 bg-amber-50/60 p-2.5 rounded-xl border border-amber-200/60 italic">
                            "{req.specialNotes}"
                          </p>
                        )}
                          ...
                          {!isConfirmed && !isCancelled ? (
                            <>
                              ...
                              >
                                ...
                                Confirm Order & Move to Orders
                              </button>
                            </>
                          ) : isConfirmed ? (
                            <button
                              onClick={() => setActiveAdminTab('orders')}
                              className="px-4 py-2 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 font-bold text-xs rounded-xl border border-indigo-200 transition"
                            >
                              View in Orders Page →
                            </button>
                          ) : (
                            <span className="text-xs font-semibold text-rose-500">Request Cancelled</span>
                          )}
                        </div>
